...
**The journal and the accounts must balance.** Every change to a page is a posting, so it needs a log entry that names the page, by path, folder or file name. `fmquery.py --balance` is the trial balance. It lists pages changed inside the journal's window that no entry mentions, and entries that name a page which no longer exists. Pages older than the first entry in `log.md` are the opening balance and are not checked.

**Every commit balances on its own.** Once the ledger is open (`fmquery.py --open-ledger`, which writes the opening commit to `.claude/ledger`), the balance also holds each commit after it to the rule. A commit that adds, changes, moves or removes a page must add a journal entry naming that page in the same commit. A posting that comes later is an efterpostering, a late entry, and counts only when it names the commit by its short hash, "the TL;DR of X was changed in 35b353e without an entry". Uncommitted changes are listed as not yet posted and do not count against the balance. The balance is what the name of this system refers to. Each fact is written twice, once by date in the journal and once by subject on its page, and the two are held against each other.
...
`.claude/settings.json` lists what the agent may do without asking. It may read the library, write in `wiki/`, `output/`, `data/` and `inbox/`, run the library's own scripts by name, and add and commit with git. Setting up may also change this file. It may not change `.claude/`, which Claude Code protects in any case, reach the network from the shell, push or add a remote, reset or clean git history, or change its own permissions. A tool that is not listed is refused when the library runs unattended, and asked about when a person is at the keyboard.
